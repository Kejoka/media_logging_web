<script lang="ts">
	...
	$: showRepeat = shouldShowRepeatIcon(medium, allMedia, 'movies');
	$: replayInfo = getReplayInfo(medium, allMedia, 'movies');
						...
						<div class="chat-start chat">
							...
							{:else if bubble.spoiler && !revealedSpoilers.includes(index)}
								<button
									...
									on:click={() => revealSpoiler(index)}
								>
									<span class="whitespace-pre-line blur-xs">{bubble.text}</span>
								</button>
							{:else}
								<div
