		...
		<div class="collapse bg-base-100">
			...
			>
				<button
					...
					on:click={() => handleImageIncrement(medium)}
				>
						...
						<div class="relative h-full overflow-hidden">
							...
							{#if medium.episode != 0}
								...
								<div
									role="button"
									tabindex="0"
									class="absolute bottom-1 left-1 flex h-6 w-6 cursor-pointer items-center justify-center rounded-full border border-base-content/10 bg-base-300/85 text-sm font-bold"
									title="Episode verringern"
									on:click={(event) => handleImageDecrement(event, medium)}
									on:keydown={(event) => {
										if (event.key === 'Enter' || event.key === ' ') {
											event.preventDefault();
											handleImageDecrement(event, medium);
										}
									}}
								>
									-
				...
				{#if showRepeat && showReplayInfo}
					...
					>
						...
						<div class="rounded-lg border border-neutral-400 bg-base-100 p-3 shadow-lg">
							<p class="font-semibold">{medium.rewatch_count || 1} Mal geschaut</p>
						</div>
					</div>
				{/if}
				<div
			...
			<div class="collapse-content px-2 pt-0">
				...
				{#if own_profile}
					<div class="mt-3 flex flex-wrap justify-evenly gap-2">
						<button
							type="button"
							class="btn btn-circle w-1/6 text-warning"
						...
						<button
							type="button"
							class="btn btn-circle w-1/6 text-accent"
						...
						<button
							type="button"
							class="btn btn-circle w-1/6 text-info"
							aria-label="Wo streamen?"
							title="Wo streamen?"
						...
						<button
							type="button"
							class="btn btn-circle w-1/6 text-primary"
						...
						<button
							type="button"
							class="btn btn-circle w-1/6 text-error"
