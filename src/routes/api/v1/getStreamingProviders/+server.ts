...
export async function POST({ request, locals: { supabase, safeGetSession } }) {
	...
	while (try_count < RETRIES) {
		try {
			if (medium === 'movies') {
				raw_res = await fetch(
					`https://api.themoviedb.org/3/movie/${tmdb_id}/watch/providers?api_key=${PRIVATE_TMDB_V3_KEY}`
				);
			} else if (medium === 'shows') {
				raw_res = await fetch(
					`https://api.themoviedb.org/3/tv/${tmdb_id}/watch/providers?api_key=${PRIVATE_TMDB_V3_KEY}`
				);
			} else {
