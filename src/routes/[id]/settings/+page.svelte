	...
	<div class="flex h-[90%] flex-col items-center justify-center">
		{#if is_online}
			<div class="mx-auto flex w-[90%] flex-col items-center justify-center">
				...
				>
					<label class="input-bordered input mb-3 flex items-center gap-2">
						<svg
					...
					<input
						class="btn-neutral-content btn w-full"
						type="submit"
				...
				<form class="mb-3 w-[75%]" method="post" action="?/signout" use:enhance={handleSignOut}>
					<div>
						<button class="btn-neutral-content btn w-full" disabled={loading}>Abmelden</button>
					</div>
				</form>
				<button
					class="btn-neutral-content btn mb-3 w-[75%]"
					disabled={loading}
					type="button"
					on:click={() => {
